...
def register_rider():
    ...
    # Verifica CF non già usato
    cf_esistente = query(
        "SELECT id FROM rider_dettagli WHERE codice_fiscale=%s",
        (cf,), fetchone=True
    )
    if cf_esistente:
    ...
    # 2. Salva dettagli rider nel DB auth
    try:
        query(
            """INSERT INTO rider_dettagli
               (id_utente, cognome, codice_fiscale, zona)
               VALUES (%s, %s, %s, %s)""",
            (nuovo_id, d["cognome"], cf, d.get("zona")),
            commit=True
        )
    ...
    # 3. Crea record nella tabella riders
    try:
        query(
            "INSERT INTO riders (id_utente, telefono, zona) VALUES (%s, %s, %s)",
            (nuovo_id, d.get("telefono"), d.get("zona")),
            commit=True
        )
    except Exception:
        pass  # Non bloccante, non blocca la registrazione
    ...
    try:
        risposta_ristorante = http.post(
            ...
            json={
                ...
                "telefono"   : d.get("telefono_ristorante"),
                "partita_iva": d.get("partita_iva"),
                "categoria"  : d.get("categoria"),
                "foto_profilo": d.get("foto_profilo")
            },
