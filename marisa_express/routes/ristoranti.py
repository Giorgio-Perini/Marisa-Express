...
def crea_ristorante():
    ...
    nuovo_id = query(
        """INSERT INTO ristoranti
           (id_utente, nome, descrizione, via, lat, lng, telefono,
            partita_iva, foto_profilo, categoria)
           VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)""",
        (request.utente_id, d["nome"], d.get("descrizione"), d["via"],
         d["lat"], d["lng"], d.get("telefono"), d.get("partita_iva"),
         d.get("foto_profilo"), d.get("categoria")),
        commit=True
    )
...
def lista_ristoranti():
    risultati = query(
        "SELECT id, nome, descrizione, via, lat, lng, "
        "telefono, foto_profilo, categoria FROM ristoranti"
    )
    for r in risultati:
...
# ── GET /api/ristoranti/vicini?lat=xx&lng=yy&raggio=30 ───────────────────────
...
def ristoranti_vicini():
    ...
    # Formula Haversine in SQL
    sql = """
        SELECT
            id, nome, descrizione, via, lat, lng,
            telefono, foto_profilo, categoria,
            ROUND(
                6371 * 2 * ASIN(SQRT(
...
def get_ristorante(rid):
    ristorante = query(
        "SELECT * FROM ristoranti WHERE id=%s", (rid,), fetchone=True
    )
    if not ristorante:
...
def update_ristorante(rid):
    ristorante = query(
        "SELECT id, id_utente FROM ristoranti WHERE id=%s", (rid,), fetchone=True
    )
    if not ristorante:
        return jsonify({"errore": "Ristorante non trovato"}), 404
    ...
    d = request.get_json(silent=True) or {}
    campi = ["nome", "descrizione", "via", "lat", "lng",
             "telefono", "partita_iva", "foto_profilo", "categoria"]
